...
module.exports = {
    ...
    authenticate: function authenticate(req, params, callback) {
        ...
        var successfullogin = function successfullogin(request, parameters, callback) {
                    ...
                    if (!err) {
                        ...
                        if (data.length === 1) {
                            password(params.password).verifyAgainst(data[0].hash, function (error, verified) {
                                if (error) throw new Error('AuthenticationService: Hash verification failed by unknown error!');
                                if (!verified || !data[0].active) {
    ...
    setDataConnectors: function setDataConnectors(_dataconnector, _readonly_dataconnector) {
        this.dataconnection = _dataconnector;
        this.readonly_dataconnection = _readonly_dataconnector;
    },

    setTokenPrivateKey: function setTokenPrivateKey(privatekey) {
        this.key = privatekey;
    },

    setTokenExpiryPeriod: function setTokenExpiryPeriod(days) {
